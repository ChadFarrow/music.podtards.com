...
const Albums = ({ feedUrl }: AlbumsProps) => {
  ...
  return (
    <div className={`min-h-screen relative ${theme === 'dark' ? 'bg-black' : 'bg-gray-50'}`}>
      ...
      {/* Top Navigation Bar - Show on desktop always, on mobile only when viewing individual album */}
      {!isMobile || currentFeedUrl ? (
        <div 
        ...
        <div className="flex items-center space-x-4">
          {/* Mobile: Show back button when viewing individual album */}
          {isMobile && currentFeedUrl ? (
            <Link
              to="/albums"
            ...
            >
              <Home size={20} className={theme === 'dark' ? 'text-white' : 'text-gray-700'} />
            </Link>
          ) : null}
      ...
      {/* Slide-out Menu - Hidden on mobile */}
      {showMenu && !isMobile && (
        <div 
